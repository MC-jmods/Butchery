...
public class GrantbutcheradvancementProcedure {
	...
	private static void execute(@Nullable Event event, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getItem() == ButcheryModItems.BONE_CLEAVER.get() || itemstack.getItem() == ButcheryModItems.IRON_CLEAVER.get() || itemstack.getItem() == ButcheryModItems.GOLD_CLEAVER.get()
			...
			if ((entity instanceof ServerPlayer _plr10 && _plr10.level() instanceof ServerLevel && _plr10.getAdvancements().getOrStartProgress(_plr10.server.getAdvancements().get(ResourceLocation.parse("butchery:butcher"))).isDone()) == false) {
				if (entity instanceof ServerPlayer _player) {
					AdvancementHolder _adv = _player.server.getAdvancements().get(ResourceLocation.parse("butchery:butcher"));
					if (_adv != null) {
