		...
		slotitem = (new Object() {
			public ItemStack getResult() {
				if (world instanceof ServerLevel _lvl) {
					List<TaxidermyRecipe> recipes = _lvl.recipeAccess().recipeMap().byType(TaxidermyRecipe.Type.INSTANCE).stream().map(RecipeHolder::value).collect(Collectors.toList());
					...
					List<TaxidermyRecipe> recipes = ButcheryModRecipeTypes.recipes.byType(TaxidermyRecipe.Type.INSTANCE).stream().map(RecipeHolder::value).collect(Collectors.toList());
					for (TaxidermyRecipe recipe : recipes) {
						List<Ingredient> ingredients = recipe.getIngredients();
						if (!ingredients.get(0).test((itemFromBlockInventory(world, BlockPos.containing(x, y, z), 0).copy())))
							continue;
