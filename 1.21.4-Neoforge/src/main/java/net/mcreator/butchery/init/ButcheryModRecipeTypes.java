...
public class ButcheryModRecipeTypes {
	...
	private static class Fetcher {
		...
		public static void syncRecipes(OnDatapackSyncEvent event) {
			event.sendRecipes(TaxidermyRecipe.Type.INSTANCE);
			event.sendRecipes(MeatgrinderjeiRecipe.Type.INSTANCE);
			event.sendRecipes(PestleandmortarrecipetypeRecipe.Type.INSTANCE);
		}
