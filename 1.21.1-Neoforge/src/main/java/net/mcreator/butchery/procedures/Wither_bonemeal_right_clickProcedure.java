			...
			if ((world.getBlockState(BlockPos.containing(x, y - 1, z))).getBlock() == Blocks.ROSE_BUSH) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("block.grass.place")), SoundSource.NEUTRAL, (float) 0.3, 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("block.grass.place")), SoundSource.NEUTRAL, (float) 0.3, 1, false);
					}
				}
			}
		}
