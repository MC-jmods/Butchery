...
public class ButcheryModJeiPlugin implements IModPlugin {
	...
	public void registerRecipes(IRecipeRegistration registration) {
		...
		List<TaxidermyRecipe> TaxidermyRecipes = recipeManager.getAllRecipesFor(TaxidermyRecipe.Type.INSTANCE).stream().map(RecipeHolder::value).collect(Collectors.toList());
		registration.addRecipes(Taxidermy_Type, TaxidermyRecipes);
		List<MeatgrinderjeiRecipe> MeatgrinderjeiRecipes = recipeManager.getAllRecipesFor(MeatgrinderjeiRecipe.Type.INSTANCE).stream().map(RecipeHolder::value).collect(Collectors.toList());
		registration.addRecipes(Meatgrinderjei_Type, MeatgrinderjeiRecipes);
		List<PestleandmortarrecipetypeRecipe> PestleandmortarrecipetypeRecipes = recipeManager.getAllRecipesFor(PestleandmortarrecipetypeRecipe.Type.INSTANCE).stream().map(RecipeHolder::value).collect(Collectors.toList());
