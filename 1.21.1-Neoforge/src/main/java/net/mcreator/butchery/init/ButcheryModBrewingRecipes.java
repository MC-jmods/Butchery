...
public class ButcheryModBrewingRecipes implements IModPlugin {
	...
	public void registerRecipes(IRecipeRegistration registration) {
		...
		brewingRecipes.add(factory.createBrewingRecipe(List.copyOf(ingredientStack), potion.copy(), potion2.copy()));
		ingredientStack.clear();
		ingredientStack = new ArrayList<ItemStack>(BuiltInRegistries.ITEM.getOrCreateTag(ItemTags.create(ResourceLocation.parse("c:sulfur"))).stream().map(item -> new ItemStack((Item) item.value())).collect(Collectors.toCollection(ArrayList::new)));
		potion.set(DataComponents.POTION_CONTENTS, new PotionContents(Potions.WATER));
