...
public class Enderdragoncut9Renderer implements BlockEntityRenderer<Enderdragoncut9BlockEntity> {
	...
	private void updateRenderState(Enderdragoncut9BlockEntity blockEntity, float partialTick) {
		int tickCount = (int) blockEntity.getLevel().getGameTime();
		renderState.ageInTicks = tickCount + partialTick;
	}

	@Override
	public void render(Enderdragoncut9BlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource renderer, int light, int overlayLight, Vec3 cameraPos) {
	...
	private static final class CustomHierarchicalModel extends Modelender_dragon_cut_9 {
		...
		public void setupBlockEntityAnim(Enderdragoncut9BlockEntity blockEntity, LivingEntityRenderState state) {
