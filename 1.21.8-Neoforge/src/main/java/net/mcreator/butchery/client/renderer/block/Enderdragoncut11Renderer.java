...
public class Enderdragoncut11Renderer implements BlockEntityRenderer<Enderdragoncut11BlockEntity> {
	...
	private void updateRenderState(Enderdragoncut11BlockEntity blockEntity, float partialTick) {
		int tickCount = (int) blockEntity.getLevel().getGameTime();
		renderState.ageInTicks = tickCount + partialTick;
	}

	@Override
	public void render(Enderdragoncut11BlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource renderer, int light, int overlayLight, Vec3 cameraPos) {
		updateRenderState(blockEntity, partialTick);
		poseStack.pushPose();
		poseStack.scale(-1, -1, 1);
		poseStack.translate(-0.5, -0.5, 0.5);
		BlockState state = blockEntity.getBlockState();
		Direction facing = state.getValue(Enderdragoncut11Block.FACING);
		switch (facing) {
			case NORTH -> {
			}
	...
	public static void registerBlockEntityRenderers(EntityRenderersEvent.RegisterRenderers event) {
		event.registerBlockEntityRenderer(ButcheryModBlockEntities.ENDERDRAGONCUT_11.get(), Enderdragoncut11Renderer::new);
	}

	private static final class CustomHierarchicalModel extends Modelender_dragon_cut_11 {
		public CustomHierarchicalModel(ModelPart root) {
			super(root);
		}

		public void setupBlockEntityAnim(Enderdragoncut11BlockEntity blockEntity, LivingEntityRenderState state) {
