			...
			if ((world.getBlockState(BlockPos.containing(x, y - 1, z))).getBlock() == Blocks.ROSE_BUSH) {
				if (!(entity instanceof Player _plr ? _plr.getAbilities().instabuild : false)) {
					if (entity instanceof LivingEntity _entity) {
						ItemStack _setstack7 = new ItemStack(ButcheryModItems.WITHER_BONE_MEAL.get()).copy();
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("block.grass.place")), SoundSource.NEUTRAL, (float) 0.3, 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("block.grass.place")), SoundSource.NEUTRAL, (float) 0.3, 1, false);
					}
				}
			}
		}
