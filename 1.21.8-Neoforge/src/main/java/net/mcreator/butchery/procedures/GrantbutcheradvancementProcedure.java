	...
	private static void execute(@Nullable Event event, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getItem() == ButcheryModItems.BONE_CLEAVER.get() || itemstack.getItem() == ButcheryModItems.IRON_CLEAVER.get() || itemstack.getItem() == ButcheryModItems.GOLD_CLEAVER.get()
			...
			if ((entity instanceof ServerPlayer _plr10 && _plr10.level() instanceof ServerLevel _serverLevel10
					&& _plr10.getAdvancements().getOrStartProgress(_serverLevel10.getServer().getAdvancements().get(ResourceLocation.parse("butchery:butcher"))).isDone()) == false) {
				if (entity instanceof ServerPlayer _player && _player.level() instanceof ServerLevel _level) {
					AdvancementHolder _adv = _level.getServer().getAdvancements().get(ResourceLocation.parse("butchery:butcher"));
					if (_adv != null) {
