...
@EventBusSubscriber
public class EnddragonDropProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if (entity instanceof EnderDragon) {
			if (ButcheryconfigConfiguration.END_DRAGON_CARCASS.get() == true) {
				if (ButcheryconfigConfiguration.SPECIFIC_TOOLS.get()) {
					if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).is(ItemTags.create(ResourceLocation.parse("forge:cleaver")))
						...
						if (entity instanceof EnderDragon) {
							ButcheryMod.queueServerWork(210, () -> {
								if (world instanceof ServerLevel _level)
									FallingBlockEntity.fall(_level, BlockPos.containing(x, y, z), ButcheryModBlocks.ENDER_DRAGON_CARCASS.get().defaultBlockState());
							});
						}
					}
