...
public class WitherdropProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		if (entity == null || sourceentity == null)
			return;
		if (entity instanceof WitherBoss || entity.getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse("forge:wither"))) || entity.getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse("c:wither")))) {
			if (ButcheryconfigConfiguration.WITHER_CARCASS.get() == true) {
						...
						if (entity instanceof WitherBoss) {
							ButcheryMod.queueServerWork(10, () -> {
								if (world instanceof ServerLevel _level)
									FallingBlockEntity.fall(_level, BlockPos.containing(x, y, z), ButcheryModBlocks.WITHER_CARCASS.get().defaultBlockState());
							});
						}
					}
