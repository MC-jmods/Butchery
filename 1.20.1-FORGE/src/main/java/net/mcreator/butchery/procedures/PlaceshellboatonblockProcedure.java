		...
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == ButcheryModBlocks.EMPTY_TURTLE_SHELL.get().asItem()) {
			if ((world.getBlockState(BlockPos.containing(x, y + 1, z))).getBlock() == Blocks.WATER) {
				...
				if ((entity instanceof ServerPlayer _plr7 && _plr7.level() instanceof ServerLevel
						&& _plr7.getAdvancements().getOrStartProgress(_plr7.server.getAdvancements().getAdvancement(ResourceLocation.parse("butchery:butcherssecrets"))).isDone()) == false) {
				...
				if ((entity instanceof ServerPlayer _plr9 && _plr9.level() instanceof ServerLevel
						&& _plr9.getAdvancements().getOrStartProgress(_plr9.server.getAdvancements().getAdvancement(ResourceLocation.parse("butchery:itsafloater"))).isDone()) == false) {
					if (entity instanceof ServerPlayer _player) {
